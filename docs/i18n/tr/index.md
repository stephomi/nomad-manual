# Genel bakış {#overview}

<!-- 
[[toc]]
::: tip
::: warning
::: danger Error
::: details
-->

::: tip Nomad'da yeni misiniz?
[Başlarken](gettingstarted.md) kılavuzunu okuyun.
:::


## İçindekiler tablosu {#table-of-contents}

| Menü                           | Simge                         | Açıklama                                            |
| :----------------------------: | :---------------------------: | :-------------------------------------------------: |
| [Dosyalar](files.md)           | ![](/icons/open.webp)        | Projelerinizi yönetin, dışa aktarın, içe aktarın vb |
| [Sahne](scene.md)              | ![](/icons/scene.webp)       | Yeni primitif ekleyin ve sahne nesnelerinizi yönetin |
| [Topoloji](topology.md)        | ![](/icons/multires.webp)    | Modelin topolojisini düzenleyin                     |
| [Malzeme](material.md)         | ![](/icons/material.webp)    | Malzeme (nesne düzeyi)                              |
| [Gölgelendirme](shading.md)    | ![](/icons/sun.webp)         | Sahnelerinizi nasıl aydınlatacağınız                |
| [Post Process](postprocess.md) | ![](/icons/postprocess.webp) | Post Process (ekran alanı görsel efektleri)         |
| [Arka Plan](background.md)     | ![](/icons/image.webp)       | Arka planı değiştirin ve referans görsel ekleyin    |
| [Kamera](camera.md)            | ![](/icons/camera.webp)      | Kamera kontrolü ve etkileşim                        |
| [Fırça İzi](stroke.md)         | ![](/icons/pencil.webp)      | Geçerli araç ayarlarını, fırça izini, düşüşü düzenleyin... |
| [Basınç](pressure.md)          | ![](/icons/pressure.webp)    | Kaleminizin basıncını yönetin                       |
| [Boyama](painting.md)	         | ![](/icons/paint.webp)       | Boyama araçları için boyama ayarları                |
| [Simetri](symmetry.md)         | ![](/icons/symmetry.webp)    | Geçerli örgünüzün ayna düzlemini yönetin            |
| [Katmanlar](layers.md)         | ![](/icons/layer.webp)       | Geçerli nesne katmanlarınızı yönetin               |
| [Ayarlar](settings.md)         | ![](/icons/cog.webp)         | 3B görünümü özelleştirin                            |
| [Arayüz](interface.md)         | ![](/icons/interface.webp)   | Arayüzü özelleştirin                                |
| [Araçlar](tools.md)            | ![](/icons/toolbox.webp)     | Tüm araçların/fırçaların listesi                    |
| [Geçmiş](history.md)           | ![](/icons/history.webp)     | Geri al/yeniden yap geçmiş yığınını yönetin         |
| [SSS](faq.md)                  | ![](/icons/faq.webp)         | Sıkça Sorulan Sorular                               |
| [İpuçları](tips.md)            | ![](/icons/manual.webp)      | İş akışı ipuçları                                   |

## Topluluk kaynakları {#community-resources}

<style>
.video-grid {
  display: grid;
  gap: 1.5rem;
  grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
}
.video-grid iframe {
  width: 100%;
  aspect-ratio: 16/9;
  border: 0;
}
</style>

<div class="video-grid">

<div>
<h3>Dave Reed</h3>
<iframe src="https://www.youtube.com/embed/Jm18bkWsAW0?si=Hnt3tdsRf_doFdZq" allowfullscreen></iframe>
</div>

<div>
<h3>Glen Southern</h3>
<iframe src="https://www.youtube.com/embed/0Mj_yietOKI?si=-XhtDM-oZcMRAfnJ" allowfullscreen></iframe>
</div>

<div>
<h3>ProcreateFX</h3>
<iframe src="https://www.youtube.com/embed/BPw16T6Ayck?si=q2WlYGp63OpukkuU" allowfullscreen></iframe>
</div>

<div>
<h3>Laurent Briere</h3>
<iframe src="https://www.youtube.com/embed/LMom2pNzJXU?si=-ucaRBlF97D6cO24" allowfullscreen></iframe>
</div>

<div>
<h3>Small Robot Studio</h3>
<iframe src="https://www.youtube.com/embed/vw3qpp81rF0?si=xs0Thy6kEGvHVi3q" allowfullscreen></iframe>
</div>

<div>
<h3>DM Art classes</h3>
<iframe src="https://www.youtube.com/embed/5VO0ZqyopGo?si=rtOLMvjwjCfPsdb4" allowfullscreen></iframe>
</div>

</div>