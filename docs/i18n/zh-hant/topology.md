...
## ![](/icons/multires.webp) 多重解析度 {#multiresolution}

![](/images/topology_multires_menu.webp)

### 什麼是多重解析度？ {#what-is-multiresolution}
...
#### 多重解析度工作流程 {#multiresolution-workflow}
...
### 多重解析度滑桿 {#multiresolution-slider}
...
### 刪除較高層級 {#delete-higher}
刪除目前顯示層級以上的所有細分層級。

### 線性細分 {#linear-subdivision}
在不進行平滑的情況下細分網格。
...
## ![](/icons/voxel.webp) 體素重拓樸 {#voxel-remesher}
...
### 保留銳利邊緣 {#keep-sharp-edges}
啟用後，新頂點會嘗試貼合原始網格上的銳利邊緣，但可能會引入變形。

## ![](/icons/dynamic.webp) 動態拓撲 {#dynamic-topology}
...
### 使用壓力影響半徑 {#use-pressure-on-radius}
僅在 `Radius` 模式下有意義。啟用後，即使筆刷大小受壓力影響，細節等級仍會反映實際筆刷大小。

### 使用筆觸衰減 {#use-stroke-falloff}
...
### 保護遮罩區域 {#protect-masked-area}
啟用後，被遮罩的區域在拓撲上不會被改變。

### 頂點外推 {#vertex-extrapolation}
...
## ![](/icons/topo_extra.webp) 其他 {#misc}

![](/images/topology_misc_menu.webp)

##### ![](/icons/cog.webp) 齒輪選單 {#gear-menu}
...
#### ![](/icons/cog.webp) UVAtlas 齒輪選單 {#uvatlas-gear-menu}
齒輪選單包含以下進階選項：

#### 面群組 {#atlas-face-group}
...
### UV 展開 - BFF {#uv-unwrap-bff}
![](/images/topology_uvbff_menu.webp)

BFF UV 採取「少切割、多變形」的策略。 

#### ![](/icons/cog.webp) UV BFF 齒輪選單 {#uv-bff-gear-menu}
...
#### ![](/icons/cog.webp) 四邊即時重拓樸齒輪選單 {#quad-remesh-instant-gear-menu}
齒輪選單包含以下進階選項：

##### 摺痕角度 {#crease-angle}
用來判斷銳利角度的閾值，會嘗試利用這些資訊來引導重拓撲。

#### 最大填補孔洞 {#max-fill-hole}
...
#### 清理 {#clean}
執行清理動作。
#### ![](/icons/cog.webp) 強制流形齒輪選單 {#force-manifold-gear-menu}
齒輪選單包含以下進階選項：

#### 刪除小面 {#delete-small-faces}
用來刪除並合併過小多邊形的閾值。
