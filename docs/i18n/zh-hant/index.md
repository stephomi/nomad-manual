# 概述 {#overview}

<!-- 
[[toc]]
::: tip
::: warning
::: danger Error
::: details
-->

::: tip 初次接觸 Nomad？
請先閱讀 [快速上手](gettingstarted.md) 指南。
:::


## 目錄 {#table-of-contents}

| Menu                           | Icon                          | Description                                        |
| :----------------------------: | :---------------------------: | :------------------------------------------------: |
| [Files](files.md)              | ![](/icons/open.webp)        | 管理你的專案、匯出、匯入等                         |
| [Scene](scene.md)              | ![](/icons/scene.webp)       | 新增基本物件並管理場景中的物件                    |
| [Topology](topology.md)        | ![](/icons/multires.webp)    | 編輯模型的拓樸結構                                |
| [Material](material.md)        | ![](/icons/material.webp)    | 材質（物件層級）                                   |
| [Shading](shading.md)          | ![](/icons/sun.webp)         | 如何為場景打光                                    |
| [Post Process](postprocess.md) | ![](/icons/postprocess.webp) | 後製處理（螢幕空間視覺效果）                      |
| [Background](background.md)    | ![](/icons/image.webp)       | 更換背景並加入參考圖片                            |
| [Camera](camera.md)            | ![](/icons/camera.webp)      | 相機控制與互動                                    |
| [Stroke](stroke.md)            | ![](/icons/pencil.webp)      | 編輯目前工具設定、筆劃、衰減等                    |
| [Pressure](pressure.md)        | ![](/icons/pressure.webp)    | 管理手寫筆的壓力反應                              |
| [Painting](painting.md)	     | ![](/icons/paint.webp)       | 繪畫工具的繪畫相關設定                            |
| [Symmetry](symmetry.md)        | ![](/icons/symmetry.webp)    | 管理目前網格的鏡射平面                            |
| [Layers](layers.md)            | ![](/icons/layer.webp)       | 管理目前物件的圖層                                |
| [Settings](settings.md)        | ![](/icons/cog.webp)         | 自訂 3D 視窗                                      |
| [Interface](interface.md)      | ![](/icons/interface.webp)   | 自訂介面                                          |
| [Tools](tools.md)              | ![](/icons/toolbox.webp)     | 所有工具／筆刷一覽                                |
| [History](history.md)          | ![](/icons/history.webp)     | 管理復原／重做的歷史堆疊                          |
| [FAQ](faq.md)                  | ![](/icons/faq.webp)         | 常見問題                                          |
| [Tips](tips.md)                | ![](/icons/manual.webp)      | 工作流程小技巧                                    |

## 社群資源 {#community-resources}

<style>
.video-grid {
  display: grid;
  gap: 1.5rem;
  grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
}
.video-grid iframe {
  width: 100%;
  aspect-ratio: 16/9;
  border: 0;
}
</style>

<div class="video-grid">

<div>
<h3>Dave Reed</h3>
<iframe src="https://www.youtube.com/embed/Jm18bkWsAW0?si=Hnt3tdsRf_doFdZq" allowfullscreen></iframe>
</div>

<div>
<h3>Glen Southern</h3>
<iframe src="https://www.youtube.com/embed/0Mj_yietOKI?si=-XhtDM-oZcMRAfnJ" allowfullscreen></iframe>
</div>

<div>
<h3>ProcreateFX</h3>
<iframe src="https://www.youtube.com/embed/BPw16T6Ayck?si=q2WlYGp63OpukkuU" allowfullscreen></iframe>
</div>

<div>
<h3>Laurent Briere</h3>
<iframe src="https://www.youtube.com/embed/LMom2pNzJXU?si=-ucaRBlF97D6cO24" allowfullscreen></iframe>
</div>

<div>
<h3>Small Robot Studio</h3>
<iframe src="https://www.youtube.com/embed/vw3qpp81rF0?si=xs0Thy6kEGvHVi3q" allowfullscreen></iframe>
</div>

<div>
<h3>DM Art classes</h3>
<iframe src="https://www.youtube.com/embed/5VO0ZqyopGo?si=rtOLMvjwjCfPsdb4" allowfullscreen></iframe>
</div>

</div>