# ![](/icons/postprocess.webp) ポストプロセス {#post-process}
...
### 最大フレームサンプリング {#max-frame-sampling}
...
## グローバルイルミネーション（SSGI） {#global-illumination-ssgi}
...
## アンビエントオクルージョン（SSAO） {#ambient-occlusion-ssao}
