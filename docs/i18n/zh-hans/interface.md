...
### 添加快捷方式（底部）... {#add-shortcuts-bottom}
...
#### ![](/icons/more.webp) 绑定 {#bindings-list}
...
#### ![](/icons/list.webp) 顺序 {#order}

显示快捷方式列表。长按并拖动以重新排序快捷方式。

#### ![](/icons/reset.webp) 重置 {#reset}

Reset 会将底部工具栏恢复为默认设置。

### 添加快捷方式（窗口）... + {#add-shortcuts-window}
...
#### ![](/icons/resize_bottom_right.webp) 界面调整角 {#ui-resize-corner}
...
#### 高对比度按钮 {#high-contrast-button}
按钮的另一种样式，使启用状态更明显。如果设置为 Auto，当启用/禁用状态的 UI 颜色对比度较低时，Nomad 会自动使用此模式。

#### 颜色控件/基础颜色 {#color-widgetcolor-base}
界面中使用的主色。

#### 透明面板、颜色面板、模糊强度 {#transparent-panel-color-panel-blur-strength}
...
### 镜像顶部栏 {#mirror-top-bar}
反转顶部栏中菜单的顺序。

### 镜像侧边栏 {#mirror-side-bars}
交换侧边栏位置，使 toolbox 在左侧，工具选项在右侧。

### 镜像底部栏 {#mirror-bottom-bar}
将底部栏移动到右下角，并反转按钮顺序。

### 材质颜色预览 {#material-color-preview}
当你为材质选择颜色时，会在当前选中对象上显示该材质的预览。

----
### 悬停时帮助弹窗 {#help-popup-on-hover}
...
### 切换 遮罩 <-> 选择遮罩 {#toggle-mask-selmask}
...
### ![](/icons/fingerprint.webp) 历史 2/3 {#history-23}
...
### ![](/icons/more.webp) 上下文菜单 {#context-menu}
...
## iOS ⌘ 键盘快捷键显示 {#ios-keyboard-shortcuts-display}
