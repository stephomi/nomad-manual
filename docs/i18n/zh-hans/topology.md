# ![](/icons/multires.webp) 拓扑 {#topology}
...
## ![](/icons/multires.webp) 多重分辨率 {#multiresolution}

![](/images/topology_multires_menu.webp)

### 什么是多重分辨率？ {#what-is-multiresolution}
多分辨率功能主要用于两种场景：
- 使用平滑细分算法提升物体的多边形数量
...
#### 多重分辨率工作流程 {#multiresolution-workflow}
...
### 多重分辨率滑块 {#multiresolution-slider}
...
## ![](/icons/voxel.webp) 体素重拓扑 {#voxel-remesher}
...
### 分辨率 {#voxel-resolution}
用于计算的体素大小。在调整该参数时，网格上会显示棋盘格图案，以预览结果。

### 构建多重分辨率 {#build-multiresolution}
...
## ![](/icons/dynamic.webp) 动态拓扑 {#dynamic-topology}
...
### 使用压感控制半径 {#use-pressure-on-radius}
仅在启用 `Radius` 模式时有效。启用后，即使笔刷大小受压感影响，细节级别也会始终反映当前笔刷大小。

### 使用笔划衰减 {#use-stroke-falloff}
...
### 保护遮罩区域 {#protect-masked-area}
启用后，被遮罩区域的拓扑不会被修改。

### 顶点外推 {#vertex-extrapolation}
...
## ![](/icons/topo_extra.webp) 杂项 {#misc}

![](/images/topology_misc_menu.webp)

##### ![](/icons/cog.webp) 齿轮菜单 {#gear-menu}
...
#### ![](/icons/cog.webp) UVAtlas 齿轮菜单 {#uvatlas-gear-menu}
齿轮菜单包含以下高级选项：
...
### UV 展开 - BFF {#uv-unwrap-bff}
![](/images/topology_uvbff_menu.webp)

BFF UV 更偏向“切缝更少、变形更多”的策略。 

#### ![](/icons/cog.webp) UV BFF 齿轮菜单 {#uv-bff-gear-menu}
...
#### ![](/icons/cog.webp) 四边面重拓扑 Instant 齿轮菜单 {#quad-remesh-instant-gear-menu}
齿轮菜单包含以下高级选项：
...
#### 清理 {#clean}
执行清理操作。
#### ![](/icons/cog.webp) 强制流形齿轮菜单 {#force-manifold-gear-menu}
齿轮菜单包含以下高级选项：

#### 删除小面 {#delete-small-faces}
用于删除并合并小多边形的阈值。
...
#### 分辨率 {#triplanar-resolution}
三平面操作中使用的体素大小。

## ![](/icons/dot.webp) 基元 {#primitive}
