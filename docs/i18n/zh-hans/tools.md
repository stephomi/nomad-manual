...
### ![](/icons/tool_brush.webp) 笔刷 {#brush}
标准笔刷。`Sub` 会移除材料。

![](/videos/tool_brush.mp4)

### ![](/icons/tool_move.webp) 移动 {#move}
...
### ![](/icons/tool_maskSelector.webp) 选择遮罩 {#selection-mask}
...
### ![](/icons/tool_crease.webp) 折痕 {#crease}
Crease 工具适合雕刻小切口或凹痕。

Sub 模式为 `Invert`，会创建凸起的折痕。

#### 折痕设置 {#crease-settings}
...
### ![](/icons/tool_pinch.webp) 收缩 {#pinch}
该工具可用于锐化边缘。
...
### ![](/icons/tool_split.webp) 切分 {#split}
...
### ![](/icons/tool_layer.webp) 图层 {#layer}
抬高表面，但限制高度。
...
### ![](/icons/tool_flatten.webp) 充气 {#inflate}
沿各自法线方向移动顶点。`Sub` 会沿反向法线移动顶点。

#### 充气设置 {#inflate-setings}
* `Relax (Normal)` - 对法线应用的平滑量。
...
### ![](/icons/tool_nudge.webp) 轻推 {#nudge}
沿笔划方向移动或“涂抹”点。
...
### ![](/icons/tool_clearLayer.webp) 删除图层 {#delete-layer}
该工具可局部重置图层，必须有激活图层，否则不会发生任何事情。

![](/videos/tool_delete_layer.mp4)


### ![](/icons/tool_tube.webp) 管状 {#tube}
通过绘制曲线创建一根管子。 
...
#### 曲线点删除切换 {#curve-point-delete-toggle}
...
### ![](/icons/tool_lathe.webp) 车削 {#lathe}
通过绘制曲线创建旋转体表面。
...
### ![](/icons/tool_faceGroup.webp) 面组 {#facegroup}
...
#### 面组左侧工具栏 {#facegroup-left-toolbar}
...
### ![](/icons/tool_measure.webp) 测量 {#measure}
拖动以测量两点之间的距离。

### ![](/icons/tool_remesh.webp) 四边形重新网格化 {#quad-remesher}
...
#### 四边形重新网格化顶部工具栏 {#quad-remesher-top-toolbar}
...
#### 四边形重新网格化设置菜单 {#quad-remesher-settings-menu}
...
### ![](/icons/tool_select.webp) 选择 {#select}
使用形状模式选择场景中的对象。`Unselect` 将从选择中移除对象。

### ![](/icons/tool_view.webp) 视图 {#view}
此“工具”本身并不执行任何特殊操作，只是用于在不修改场景的情况下查看模型。


## 工具箱上下文菜单 {#toolbox-context-menu}
